...
function BlogCopyright({ page, link }) {
  return (
    <div className="bg-light-400 dark:bg-dark-400 relative -mb-4 mt-4 overflow-hidden rounded p-4 text-sm md:-mx-4 md:rounded-none">
      ...
      <div className="font-serif text-lg leading-6">{page.properties.name.title[0].plain_text} - RealTong</div>
      ...
      <div className="my-4 grid grid-cols-3 opacity-90 md:w-2/3">
        ...
        <div>
          ...
          <div>{new Date(page.properties.date.date.start).toLocaleDateString()}</div>
        </div>
        <div>
      ...
      <p className="font-bold leading-6">署名、非商业性和共享性。</p>
    </div>
  )
}
